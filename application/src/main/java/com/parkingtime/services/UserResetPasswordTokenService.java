...
import com.parkingtime.repositories.UserResetPasswordHistoryRepository;
import com.parkingtime.repositories.UserResetPasswordTokenRepository;
...
public class UserResetPasswordTokenService {
    ...
    public MessageResponse resetPassword(ResetPasswordRequest request) {
        ...
        if(Boolean.TRUE.equals(userResetPasswordToken.getUsedToken()) ||
            ...
            log.error("User is trying to input expired token or already used token");
            throw new IllegalArgumentException(ErrorMessages.TOKEN_EXPIRED.getValue());
        }

        if(!request.getNewPassword().equals(request.getConfirmPassword())) {
            log.error("User is trying to input mismatch password");
